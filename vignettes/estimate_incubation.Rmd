...
The function `fit_gamma_incubation_dist()` takes the same inputs, but directly fits a discrete gamma distribution by sampling the empirical distribution and fitting a discrete gamma distribution to it by the means of `fit_disc_gamma`.
...
mkexposures <- function(foo) foo - base::sample(x, size = sample.int(5, size = 1), replace = FALSE, prob = y)
